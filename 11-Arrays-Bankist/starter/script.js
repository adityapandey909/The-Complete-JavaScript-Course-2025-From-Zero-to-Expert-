...
// const movementsDescriptions = movements.map((mov, i) => `Movement ${i + 1}: You ${mov > 0 ? 'deposited' : 'withdrawal'} ${Math.abs(mov)}`
// );
...
// const balance = movements.reduce((acc, curr) => acc + cur, 0);
// console.log(balance);
...
// console.log(`Your latest large movement was ${movements.length - latestLargeMovementIndex} moments ago`);
...
// const groupedMovements = Object.groupBy(movements, movement => movement > 0 ? 'deposit' : 'withdrawls');
...
//   .reduce(
//     (sums, cur) => {
//       // cur > 0 ? (sums.deposits += cur) : (sums.withdrawals += curr);
